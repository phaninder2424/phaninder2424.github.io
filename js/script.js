$(document).ready(function() {
  ...
  $(function() {
    element.typed({
      strings: ["an innovative thinker", "a machine learning engineer.","also a Bussiness Analyst.", "a Tech-developer too.",
      ],
      typeSpeed: 100,
      loop: true,
    });
  ...
  $(window).on('scroll', function() {
    ...
    if (wScroll > (skill.offset().top - 400)) {
      skill.each(function(i) {
        setTimeout(function() {
          skill.eq(i).find('.progress-bar').attr('style', 'width: ' + skill.eq(i).find(
            'li.strength').text() + '');
        }, 200 + (200 * i));
      });
    }
